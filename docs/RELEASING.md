...
릴리스 표시 이름과 Git 태그는 모두 배포 EXE 파일명과 동일하게 지정합니다(예: `DevBootstrap-0.1.2-x64.exe`).
...
저장소 루트에서 실행합니다. 아래 `0.1.2`은 다음 버전 예시이며 이미 배포한 버전을 재사용하지 않습니다.
...
`docs/releases/v0.1.2.md`에 변경 동작, 설치 순서, 검증 결과와 남은 한계를 작성합니다.
...
git add <검토한 파일들>
git commit -m "release: prepare dev bootstrap v0.1.2"
git push origin main
git tag -a DevBootstrap-0.1.2-x64.exe -m "DevBootstrap-0.1.2-x64.exe"
git push origin DevBootstrap-0.1.2-x64.exe
```
...
```bash
(cd app/release && sha256sum DevBootstrap-0.1.2-x64.exe > SHA256SUMS.txt)
gh release create DevBootstrap-0.1.2-x64.exe \
  app/release/DevBootstrap-0.1.2-x64.exe app/release/SHA256SUMS.txt \
  --verify-tag --draft --prerelease \
  --title "DevBootstrap-0.1.2-x64.exe" \
...
```bash
gh release view DevBootstrap-0.1.2-x64.exe --json url,assets,isDraft,isPrerelease
gh release edit DevBootstrap-0.1.2-x64.exe --draft=false
```
