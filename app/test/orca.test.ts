...
const asset={name:'orca-windows-setup.exe',size:body.length,digest:'sha256:'+createHash('sha256').update(body).digest('hex'),browser_download_url:'https://github.com/stablyai/orca/releases/download/v1.0/orca-windows-setup.exe'};
...
test('Orca installer metadata requires official URL and verifiable stable asset',()=>{
 ...
 for(const change of [{digest:null},{size:-1},{browser_download_url:'https://example.com/orca-windows-setup.exe'}])assert.throws(()=>installerAsset({...release,assets:[{...asset,...change}]}));
