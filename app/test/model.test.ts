...
test('MuRing defaults preserve user settings and exclude unsafe/conditional actions',()=>{
  ...
  assert(c.selected.includes('kb')); assert(c.selected.includes('orca'));
  for(const id of ['shell-replace','claude-permissions','orca-patch','git-name'])assert(!c.selected.includes(id));
...
test('dependency selection and cascading removal',()=>{
  ...
  assert(!selected.includes('codex')); assert(!selected.includes('kb')); assert(selected.includes('gh'));
...
test('recommendations respond to profile and zsh selection',()=>{
  ...
  assert(!recommended(catalog.find(i=>i.id==='kb')!,c));
  assert(recommended(catalog.find(i=>i.id==='node')!,c));
  assert(!recommended(catalog.find(i=>i.id==='shell-theme')!,c));
  c.selected.push('zsh');assert(recommended(catalog.find(i=>i.id==='shell-theme')!,c));
});
