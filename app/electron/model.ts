...
export interface Config { version: 1; profile: Profile; selected: string[]; distro: string; user: string; installLocation: string; gitName: string; gitEmail: string; timezone: string; kbRepo: string; kbDir: string; contentCommit: string }
export const CONTENT_GROUPS = ['claude-skill','claude-commands','codex-skills'];
export interface ContentPreview { commit: string; installedCommit: string; message: string; date: string; commands: string[]; skills: string[]; changes: {path: string; status: string}[] }
...
export function validate(raw: unknown, catalog: Item[]): Config {
  ...
  if (value.contentCommit && !/^[0-9a-f]{40}$/.test(value.contentCommit)) throw Error('커맨드·스킬 커밋을 다시 확인하세요.');
