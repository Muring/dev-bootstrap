...
export function installerAsset(release: any) {
  ...
  if (
    ...
    throw Error('공식 Orca 설치 파일의 크기·체크섬을 확인할 수 없습니다.');
  const url = new URL(asset.browser_download_url);
  if (
    url.origin !== 'https://github.com' ||
    !url.pathname.startsWith('/stablyai/orca/releases/download/') ||
    !url.pathname.endsWith('/orca-windows-setup.exe') ||
    url.search ||
    url.hash
  )
    throw Error('공식 Orca 다운로드 주소가 아닙니다.');
  return {
    url: url.href,
    size: asset.size,
...
export async function downloadOrca(
  directory: string,
  fetcher: typeof fetch = fetch,
  report: (update: ProgressUpdate) => void = () => {},
) {
  report({ label: 'Orca 공식 최신 안정판과 체크섬 확인' });
  const response = await fetcher('https://api.github.com/repos/stablyai/orca/releases/latest', {
    headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'dev-bootstrap' },
  ...
  try {
    ...
    source.on('data', (chunk: Buffer) => {
      size += chunk.length;
      report({ label: 'Orca 설치 파일 다운로드', completed: size, total: asset.size, unit: 'bytes' });
      if (size > asset.size) source.destroy(Error('Orca 설치 파일 크기가 다릅니다.'));
    });
    ...
    if (size !== asset.size || hash.digest('hex') !== asset.sha256)
      throw Error('Orca 설치 파일 체크섬 검증에 실패했습니다. 실행하지 않았습니다.');
